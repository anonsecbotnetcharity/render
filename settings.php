...
if (!file_exists("phash")){
	...
	$passHash = isset($_COOKIE['phash']) ? $_COOKIE['phash'] : '';
	if (md5("randomsalt".$passHash) != $storedPassHash){
		header("Location: login.php");
	} else {

		$proxies = isset($_POST['proxies']) ? $_POST['proxies'] : '';
		$blogs = isset($_POST['blogs']) ? $_POST['blogs'] : '';
		$check = isset($_POST['check']) ? $_POST['check'] : '';

		if ($check == "ok"){
			$proxyFile = "proxy";
			$fh = fopen($proxyFile, 'w+');
			fwrite($fh, $proxies);
			fclose($fh);
			$blogFile = "blog";
			$fh = fopen($blogFile, 'w+');
	...
	<div class="form-group">
		...
		<textarea rows="10" class="form-control" placeholder="proxylist" type="text" name="proxies"><?php
			...
			if (file_exists($proxyFile)) {
				...
				$content = fread($fp, filesize($proxyFile));
				fclose($fp);
				echo $content;
			}
		?></textarea>
    ...
    <div class="form-group">
		...
		<textarea rows="10" class="form-control" placeholder="bloglist" type="text" name="blogs"><?php
			...
			if (file_exists($blogFile)) {
				$fp = fopen($blogFile, "r");
				$content = fread($fp, filesize($blogFile));
				fclose($fp);
				echo $content;
			}
		?></textarea>
